...
sys.path.append('/opt/MultimodalitySurvivalPrediction/models')
...
for modalities in modalities_list:
    ...
    for k, (train, test_val) in enumerate(train_testVal_kfold):
        ...
        prefix = f'pancancer_fold_{k}_{run_tag}'
        epoch = 'epoch=last'
        weight_dir = 'trained_model'
        file_name = os.path.join(
            weight_dir,
            f'{prefix}_{epoch}_c_index{test_c_index.item():.2f}.pth'
        )
        torch.save(survmodel.model.state_dict(), file_name)
